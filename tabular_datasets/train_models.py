...
HYPERPARAMETERS = {
    'sgd_test': {'learning_rate': ['constant'],
            'eta0': [0.00001], 
            'alpha': [0.00001],
            'loss': ['log_loss'],
            'penalty':['l1'], 
    ...
    'mlp_two_layer': {'hidden_layer_sizes':
            ...
            'n_iter_no_change': [10],
            'tol': [1e-4]},
    'mlp_three_layer': {'hidden_layer_sizes':
                                [(500, 500, 500), (500, 500, 100), (500, 500, 10),
                                (500, 100, 500), (500, 100, 100), (500, 100, 10),
                                (500, 10, 500), (500, 10, 100), (500, 10, 10),
                                (100, 500, 500), (100, 500, 100), (100, 500, 10),
                                (100, 100, 500), (100, 100, 100), (100, 100, 10),
                                (100, 10, 500), (100, 10, 100), (100, 10, 10),
                                (10, 500, 500), (10, 500, 100), (10, 500, 10),
                                (10, 100, 500), (10, 100, 100), (10, 100, 10),
                                (10, 10, 500), (10, 10, 100), (10, 10, 10)],
            'learning_rate_init': [0.00001, 0.0001, 0.001, 0.01, 0.1, 1],
            'max_iter': [1000],
            'early_stopping': [True],
            'n_iter_no_change': [10],
    ...
    for model in args.models:
        ...
        for ds in datasets.keys():
            ...
            for id_scenario in model_sets.keys():
                ...
                log_loss_test = fm.log_loss(Y_test[np.unique(np.where(~np.isnan(Y_pred_prob))[0])], Y_pred_prob[np.unique(np.where(~np.isnan(Y_pred_prob))[0])])
                demp = None if s_v_family else fm.demographic_parity_dif(Y_pred, S_test, 1) 
                eqopp = None if s_v_family else fm.equal_opp_dif(Y_test, Y_pred, S_test, 1)
                eqodd = None if s_v_family else fm.equal_odd_dif(Y_test, Y_pred, S_test, 1)
                ...
                new['f1_train_mean'], new['f1_train_sd'] = (st.mean(scores['test_f1_macro']), st.stdev(scores['test_f1_macro'])) if len(labels)>2 else (st.mean(scores['test_f1']), st.stdev(scores['test_f1']))
...
if __name__=='__main__':
    ...
    parser.add_argument('--n_jobs', type=int, default=-1, help='number of core to parallelize jobs')
