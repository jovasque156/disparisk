...
MODELS = ['sgd_lr', 'mlp_one_layer', 'mlp_two_layer', 'mlp_three_layer']
...
def compute_results_plots(model, data, data_sets, metric):
    ...
    pve_y, pve_y_pos, pve_y_neg, prob_pred_y = v_info_scores['pve_y']
    pve_y_all, pve_y_all_pos, pve_y_all_neg, prob_pred_y_all = v_info_scores['pve_y_all']
    pve_y_s, pve_y_s_pos, pve_y_s_neg, prob_pred_y_s = v_info_scores['pve_y_s']
    pve_y_x, pve_y_x_pos, pve_y_x_neg, prob_pred_y_x = v_info_scores['pve_y_x']

    s_v_info_scores = pickle.load(open(f'v_info_scores/{model}/{data}_s_v_info.pkl', 'rb'))    
    pve_s, pve_s_pos, pve_s_neg, prob_pred_s = s_v_info_scores['pve_s']
    pve_s_all, pve_s_all_pos, pve_s_all_neg, prob_pred_s_all = s_v_info_scores['pve_s_all']
    pve_s_y, pve_s_y_pos, pve_s_y_neg, prob_pred_s_y = s_v_info_scores['pve_s_y']
    pve_s_x, pve_s_x_pos, pve_s_x_neg, prob_pred_s_x = s_v_info_scores['pve_s_x']

    if metric=='pvi_from_s_given_x':
        metric_pve = pve_y_x - pve_y_all
    elif metric=='pvi_from_s_given_x_weighted':
        pos_weight = pve_y_x_pos*prob_pred_y_x[:,1] - pve_y_all_pos*prob_pred_y_all[:,1]
        neg_weight = pve_y_x_neg*prob_pred_y_x[:,0] - pve_y_all_neg*prob_pred_y_all[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_from_x_given_s':
        metric_pve = pve_y_s - pve_y_all
    elif metric=='pvi_from_x_given_s_weighted':
        pos_weight = pve_y_s_pos*prob_pred_y_s[:,1] - pve_y_all_pos*prob_pred_y_all[:,1]
        neg_weight = pve_y_s_neg*prob_pred_y_s[:,0] - pve_y_all_neg*prob_pred_y_all[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_from_s':
        metric_pve = pve_y - pve_y_s
    elif metric=='pvi_from_s_weighted':
        pos_weight = pve_y_pos*prob_pred_y[:,1] - pve_y_s_pos*prob_pred_y_s[:,1]
        neg_weight = pve_y_neg*prob_pred_y[:,0] - pve_y_s_neg*prob_pred_y_s[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_from_x_and_s':
        metric_pve = pve_y - pve_y_all
    elif metric=='pvi_from_x_and_s_weighted':
        pos_weight = pve_y_pos*prob_pred_y[:,1] - pve_y_all_pos*prob_pred_y_all[:,1]
        neg_weight = pve_y_neg*prob_pred_y[:,0] - pve_y_all_neg*prob_pred_y_all[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_from_x':
        metric_pve = pve_y-pve_y_x
    elif metric=='pvi_from_x_weighted':
        pos_weight = pve_y_pos*prob_pred_y[:,1] - pve_y_x_pos*prob_pred_y_s[:,1]
        neg_weight = pve_y_neg*prob_pred_y[:,0] - pve_y_x_neg*prob_pred_y_s[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_s_from_x':
        metric_pve = pve_s-pve_s_x
    elif metric=='pvi_s_from_x_weighted':
        pos_weight = pve_s_pos*prob_pred_s[:,1] - pve_s_x_pos*prob_pred_s_x[:,1]
        neg_weight = pve_s_neg*prob_pred_s[:,0] - pve_s_x_neg*prob_pred_s_x[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_s_from_x_and_s':
        metric_pve = pve_s-pve_s_all
    elif metric=='pvi_s_from_x_and_s_weighted':
        pos_weight = pve_s_pos*prob_pred_s[:,1] - pve_s_all_pos*prob_pred_s_all[:,1]
        neg_weight = pve_s_neg*prob_pred_s[:,0] - pve_s_all_neg*prob_pred_s_all[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_s_from_x_given_y':
        metric_pve = pve_s_x-pve_s_all
    elif metric=='pvi_s_from_x_given_y_weighted':
        pos_weight = pve_s_x_pos*prob_pred_s_x[:,1] - pve_s_all_pos*prob_pred_s_all[:,1]
        neg_weight = pve_s_x_neg*prob_pred_s_x[:,0] - pve_s_all_neg*prob_pred_s_all[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_s_from_y_given_x':
        metric_pve = pve_s_y-pve_s_all
    elif metric=='pvi_s_from_y_given_x_weighted':
        pos_weight = pve_s_y_pos*prob_pred_s_y[:,1] - pve_s_all_pos*prob_pred_s_all[:,1]
        neg_weight = pve_s_y_neg*prob_pred_s_y[:,0] - pve_s_all_neg*prob_pred_s_all[:,0]
        metric_pve = pos_weight + neg_weight
    elif metric=='pvi_s_from_y':
        metric_pve = pve_s-pve_s_y
    elif metric=='pvi_s_from_y_weighted':
        pos_weight = pve_s_pos*prob_pred_s[:,1] - pve_s_y_pos*prob_pred_s_y[:,1]
        neg_weight = pve_s_neg*prob_pred_s[:,0] - pve_s_y_neg*prob_pred_s_y[:,0]
        metric_pve = pos_weight + neg_weight
    ...
    if 'weighted' in metric:
        pd.DataFrame({'pve': metric_pve, 'pve_pos': pos_weight, 'pve_neg': neg_weight}).to_csv(f'pvi_results/{model}/{data}/{metric}.txt')
    else:
        pd.DataFrame(metric_pve, columns=['pve']).to_csv(f'pvi_results/{model}/{data}/{metric}.txt')

    results_plots = pd.DataFrame(columns = ['id_scenario', 'threshold', 'metric', 'value_metric', 'acc_test', 'acc_train_mean', 'log_loss_test', 'log_loss_train_mean'])
    results_groups_plots = pd.DataFrame(columns = ['id_scenario', 'threshold', 'metric', 'high_low', 'value_metric', 'acc_test', 'acc_train_mean', 'log_loss_test', 'log_loss_train_mean'])
    ...
    for scenario in list(results_models['id_scenario']):
            ...
            acc_train = results_models[results_models['id_scenario']==scenario]['acc_train_mean'][scenario]
            log_loss_test = results_models[results_models['id_scenario']==scenario]['log_loss_test'][scenario]
            log_loss_train = results_models[results_models['id_scenario']==scenario]['log_loss_train_mean'][scenario]
            
            acc_test_all = list([acc_test]) *len(thresholds)
            acc_train_all = list([acc_train]) *len(thresholds)
            log_loss_test_all = list([log_loss_test])*len(thresholds)
            log_loss_train_all = list([log_loss_train])*len(thresholds)
            ...
            odd = {'id_scenario': scenario_all,
                    ...
                    'acc_train_mean': acc_train_all,
                    'log_loss_test': log_loss_test_all,
                    'log_loss_train_mean': log_loss_train_all,
                    'portion_pos': portion_pos,
                    'portion_unpr': portion_unpr
                    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--models', nargs='+', type=str, default= 'sgd mlp random_forest knn' ,help='v-models to compute results')
    parser.add_argument('--datasets', nargs='+', type=str, default='dutch_census', help='Datasets to compute results')
