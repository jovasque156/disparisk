...
from models import resnet18, ResidualBlock, AlexNet, OneMLP, TwoMLP, ThreeMLP, Linear
from utils import *
...
def get_args():
    ...
    parser.add_argument('--data-dir', type=str, default='path/to/celeba.hdf5')
    parser.add_argument('--result-dir', type=str, default='path/to/results')
    parser.add_argument('--batch-size', type=int, default=512)
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--num-workers', type=int, default=8)
    parser.add_argument('--model', type=str, default="resnet18", choices=["resnet18", "alexnet","1mlp","2mlp","3mlp","linear"])
...
def compute_pvi(args):
    ...
    elif '3mlp' in args.model:
        g = ThreeMLP(num_classes=num_class)
        g_prime = ThreeMLP(num_classes=num_class)
    ...
    for e in range(args.epoch_start, args.epoch_end+1):
        ...
        checkpoint = torch.load(f'path/to/results/checkpoints/{model_from_X}_epoch{e}.pth.tar', map_location=device)
        g.load_state_dict(checkpoint["predictor"])
        checkpoint = torch.load(f'path/to/results/checkpoints/{model_from_C}_epoch{e}.pth.tar', map_location=device)
        ...
        if args.save_perfo:
            results.to_csv(f'{args.result_dir}/performances/{args.model}/perfo_{args.model}_target{target}_from{"".join([x for x in args.from_x])}_given{"".join([x for x in args.given_C])}_epoch{e}.csv')
        ...
        if args.save_pvi:
            ...
            pd.DataFrame({'index': index_X,
                            ...
                            'pvi': pvi}).to_csv(f'{args.result_dir}/pvis/{args.model}/pvis_{args.model}_target{target}_from{"".join([x for x in args.from_x])}_given{"".join([x for x in args.given_C])}_epoch{e}.csv')
